...
def test_exercise_create(client, db_session, user):
    user_id = str(user.id)
    create_path_url = EXERCISES_ENDPOINT + router.url_path_for("exercise_create") + f"?user_id={user_id}"
    data = {
        "title": "test",
    ...
    assert db_session.query(Exercise).filter(Exercise.user_id == int(user_id)).count() == 1
